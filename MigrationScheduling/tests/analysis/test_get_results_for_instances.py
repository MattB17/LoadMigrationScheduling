...
def test_with_no_files(mock_extract, mock_build):
    results = ["result0.\n", "result1.\n", "results2.\n"]
    get_results_for_instances(results, [], "migrations", "/empty/dir")
...
def test_with_1_file(mock_extract, mock_build):
    results = ["result5.\n"]
    get_results_for_instances(results, ["file7.txt"], "file", "/random/dir")
    assert results == ["result5.\n", "result7.\n"]
    mock_extract.assert_called_once_with("file7.txt", "file")
    mock_build.assert_called_once_with("/random/dir", "file7.txt", 7, False)
...
def test_with_multi_file(mock_extract, mock_build):
    ...
    build_calls = [call("/yet/another/dir", "migrations3.txt", 3, False),
                   call("/yet/another/dir", "migrations4.csv", 4, False),
                   call("/yet/another/dir", "migrations5.dat", 5, False)]
