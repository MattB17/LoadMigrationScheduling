...
def test_with_no_rounds(mock_round, mock_find, mock_migration):
    ...
    result_rounds, result_cnt = schedule_migration_in_earliest_round(
        [], 0, mock_migration, CONTROLLER_CAPS1, QOS_CAPS1, False)
    mock_find.assert_called_once_with([], 0, mock_migration, False)
    mock_round.assert_called_once_with(0, CONTROLLER_CAPS1, QOS_CAPS1)
    new_round.schedule_migration.assert_called_once_with(
...
def test_with_one_round(mock_round, mock_find, mock_migration):
    ...
    result_rounds, result_cnt = schedule_migration_in_earliest_round(
        [round], 1, mock_migration, CONTROLLER_CAPS1, QOS_CAPS1, True)
    mock_find.assert_called_once_with([round], 1, mock_migration, True)
    mock_round.assert_not_called()
    round.schedule_migration.assert_called_once_with(mock_migration, True)
...
def test_with_multi_round_can_fit(mock_round, mock_find, mock_migration):
    ...
    result_rounds, result_cnt = schedule_migration_in_earliest_round(
        rounds, 3, mock_migration, CONTROLLER_CAPS2, QOS_CAPS2, False)
    mock_find.assert_called_once_with(rounds, 3, mock_migration, False)
    mock_round.assert_not_called()
    rounds[1].schedule_migration.assert_called_once_with(
        mock_migration, False)
...
def test_with_multi_round_cant_fit(mock_round, mock_find, mock_migration):
    ...
    new_round.schedule_migration = MagicMock(side_effect=None)
    result_rounds, result_cnt = schedule_migration_in_earliest_round(
        rounds, 5, mock_migration, CONTROLLER_CAPS2, QOS_CAPS2, True)
    mock_find.assert_called_once_with(rounds, 5, mock_migration, True)
    mock_round.assert_called_once_with(5, CONTROLLER_CAPS2, QOS_CAPS2)
    new_round.schedule_migration.assert_called_once_with(mock_migration, True)
